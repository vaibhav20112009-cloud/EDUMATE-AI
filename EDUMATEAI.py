...
# Gemini image-generation model. This is separate from the fast text model.
IMAGE_MODEL = "gemini-3.1-flash-image"

def wants_image_generation(text):
    """Detect clear requests to create/draw/generate an image."""
...
def gemini_generate_image(user_text):
    ...
    prompt = (
        "Create the image requested by the student. "
        "Follow the request closely. Make the result clear, useful, "
        "and appropriate for an educational AI assistant.\n\n"
...
for message in st.session_state.messages:
    ...
    if message["role"] == "user":
        ...
        with st.chat_message(
            ...
            generated_image_path = message.get("generated_image_path")
            if generated_image_path and os.path.exists(generated_image_path):
                st.image(
                    generated_image_path,
                    caption="🎨 Generated by EduMate AI",
                    use_container_width=True
                )
...
if submit:
    ...
    try:
        ...
        if image_file is None and wants_image_generation(final_text):

            with st.spinner(
                "🎨 EduMate AI is generating your image..."
            ):

                generated_bytes = gemini_generate_image(final_text)
                ...
                ai_response = (
                    "Done! 🎨 I generated the image you requested."
                )
        ...
        if not (
            image_file is None
            and wants_image_generation(final_text)
